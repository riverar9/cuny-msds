...
setwd("F:/git/cuny_msds/data606-statistics-and-probability-for-data-analytics/labs/lab-1")
```
...
However, printing the whole data set in the console is not that useful. One advantage of RStudio is that it comes with a built-in data viewer. Click on the name `arbuthnot` in the *Environment* pane (upper right window) that lists the objects in your environment. This will bring up an alternative display of the data set in the *Data Viewer* (upper left window). You can close the data viewer by clicking on the `x` in the upper left hand corner.
...
- The first argument is always the data set. 
- Next, you provide the variables from the data set to be assigned to `aes`thetic elements of the plot, e.g. the x and the y axes. 
...
```{r thoughts - girls baptized trend over the years}
girls_graph <- ggplot(data = arbuthnot, aes(x = year, y = girls)) + geom_point()
girls_graph
```

There is a significant drop in baptisms starting in 1640 through around 1665. Looking online, it looks like the beginning of this period coincides with the English Civil Wars (1642 - 1651). From . Quickly after the 1665, the baptisms for girls returns back to levels that seem to visually align with the trend before the drop.

```{r thoughts - adding a vertical line at 1642 and 1665}
girls_graph + 
  geom_vline(xintercept = 1642) + 
  geom_vline(xintercept = 1651)
```

The English Civil Wars is a pretty convenient event to explain some of the reduction of births, but the number is sustained at a lower level. From what I found through [**english-heritage.org.uk**](https://www.english-heritage.org.uk/learn/histories/the-english-civil-wars-history-and-stories/#:~:text=The%20human%20cost%20of%20the,were%20complex%20and%20many%2Dlayered.), it seems that 4.5% of the population was lost, so I'm expecting that there would be a pretty similar drop between the year before the war and the year the war ended.

```{r thoughts - testing out a theory that the pre war christenings (1641) and post war christinings (1665) would be around a 4.5% drop relative to the 1641 births}
pre_war_girls = arbuthnot[arbuthnot$year == 1641,]$girls
post_war_girls = arbuthnot[arbuthnot$year == 1665,]$girls

print(100*(pre_war_girls - post_war_girls)/pre_war_girls)

```

6.5% is a bit more than I anticipated but that is the difference in girls births between those years.
...
or you can compute this for all years simultaneously and append it to the data set
...
It seems that boy ratio is the proportion of boys born compared to the total population. So I'll be graphing that as the y-axis and year on the x-axis

```{r generating a plot of the proporting of boys born over time}
boy_proportion_graph <- ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) + 
  geom_line()
boy_proportion_graph
```

Initially, the graph looks like it has a lot of "noise" being that there are pretty significant peaks and valleys. But, looking a little closer, it seems that the majority of the values remain between 50% and 54%, which is relatively tight of a band.

```{r regenerating the graph with y limits being .40 and .60}
boy_proportion_graph + 
  ylim(.4, .6)
```

Revisiting the the graph visualized this way it seems quite a bit more stable. It does stand out that there is no year where girls had more births than boys.
...
Firstly, let's see the first few rows of data to inspect the data set:
```{r get year spread of the present data set}
head(present)
```

From the head, we can see that the columns in this data set are *year*, *boys*, and *girls*. This is similar to the Arbuthnot data set.

To see the size of the data set, we can use the glimpse function as before:

```{r Seeing the size of the data set}
glimpse(present)
```

With the glimpse from above, we can see that there are 63 rows (corresponding to years) and 3 columns. 

To get the years included in this data set, I'll use the summarize function that was just introduced replacing min(boys) and max(boys) with years.
```{r summarizing the present data set}
present %>%
  summarize(min = min(year), max = max(year))
```

With that code above we can see that the first year with data is 1940 and the latest year of data is 2002.

So in summary:

  1) The data starts in 1940 and ends in 2002
  2) The data set has 63 rows and 3 columns
  3) The data set columns are *year*, *boys*, and *girls*
...
In counts, this data set has the same number of columns but fewer years being observed, i.e. fewer rows. Arbuthnot's dataset had 82 observations while the present dataset had 63.

Regarding the magnitude, let's use a plot to answer this question. We will plot both of these data sets. Since our question is simply to answer to the magnitude, let's graph both of these data sets using the index as the x-axis. This would make it so that we can pretty easily see how large the gap between the data sets are.

To do this, we will first need the total number of births added as a column on the present data set:

```{r calculating total for the present data set}
present <- present %>%
  mutate(total = boys + girls)
...
Now we can graph these data sets on top of each other:

```{r plotting both data sets on one chart with the index as the x-axis}
present_vs_arbuthnot = ggplot() + 
  geom_line(data = present, aes(x = as.numeric(row.names(present)), y = total), color = 'blue') + 
  geom_line(data = arbuthnot, aes(x = as.numeric(row.names(arbuthnot)), y = total), color = 'red')

present_vs_arbuthnot
```

Let's be honest. If you had asked me prior to looking at data if the magnitude of births from London in the 17th century was similar to the **total number of births in the USA in the 19th century**, I would have instinctively known it wasn't (although this was good practice). The graph generated very much so supports this. The red line (representing Arbuthnot's data) barely registers when sharing a scale with the total births in the USA in the 19th century.

Now, looking for a specific answer to the question of how large the magnitude is:

```{r calculating how large the magnitude of difference is}
max_arbuthnot = max(arbuthnot$total)
min_present = min(present$total)

magnitiude_ratio = min_present/max_arbuthnot

magnitiude_ratio

```

From that code block above, the magnitude difference between the year with the minimum total present births and maximum Arbuthnot births is 146.2, meaning that the total number of present day births is 146.2 times larger than what Arbuthnot recorded.
...
Let's first calculate the boy ratio:
```{r calculating the boy ratio}
present <- present %>%
  mutate(boy_ratio = boys/total)
...
Now, let's graph the proportion of boys over time for the present data set:
```{r graphing the the proportion of boys for the present data set}
present_boy_ratio <- ggplot(data = present, aes(x = year, y = boy_ratio)) + 
  geom_point() +
  ylim(.4, .6)
...
This seems to also align with Arbuthnot's observation. For every year in the present data set, we can see that boys consistently have more births than girls.

To see this with Arbuthnot's data, let's plot a graph where we can see both of these:

```{r plotting both data sets proportion of boy births on the same graph.}
present_vs_arbuthnot = ggplot() + 
  geom_point(data = present, aes(x = as.numeric(row.names(present)), y = boy_ratio), color = 'blue') + 
  geom_point(data = arbuthnot, aes(x = as.numeric(row.names(arbuthnot)), y = boy_ratio), color = 'red') +
  ylim(.45, .55)

present_vs_arbuthnot
```

Now this is pretty interesting! With more data set,s we can see that the present data set seems to converge much closer to a single number. Assuming that there's a specific probability that a birth would be a boy or girl, this would make a lot of sense as the more observations would converge towards to the probability of an event. Let's wrap up here by averaging the boy proportion in the present data set:

```{r getting the average boy proporition in the present data set}
mean(present$boy_ratio)
```

According to that cell, the mean of boy_ratios is 0.512516. So, it's slightly more likely for a boy to be born.
...
I appreciate the suggestion, but I prefer to do it in the following way:
  
  1) Get the maximum total across the data set
  
  2) Filter the data set to the entry with the max total
  
With those 2 steps, we can reliably get the max in the data set and present it

```{r getting the year in the present data set with the max births}
present <- present %>%
  mutate(total = boys + girls)

max_total = max(present$total)

present[(present$total == max_total),]
```

From that code block above, we can see that the year with the max births was 1961.
