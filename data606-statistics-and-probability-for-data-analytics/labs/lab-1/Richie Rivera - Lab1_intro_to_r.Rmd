...
**Insert your answer here**
```{thoughts - girls baptized trend over the years}
...
```{thoughts - adding a vertical line at 1642 and 1665}
girls_graph + geom_vline(xintercept = 1642) + geom_vline(xintercept = 1651)
```
...
```{thoughts - testing out a theory that the pre war christenings (1641) and post war christinings (1665) would be around a 4.5% drop relative to the 1641 births}
...
6.5% is a bit more than I anticipated but that seems to be the decline.
...
```{generating a plot of the proporting of boys born over time}
...
```{regenerating the graph with y limits being 40% and 60%}
boy_proportion_graph + ylim(.4, .6)
```

Revisiting the the graph visualized this way it seems quite a bit more stable. Perhaps pretty important is that there is no year where girls had more births than boys.
...
**Insert your answer here**
Firstly, let's see the first few rows of data to inspect the dataset:
```{get year spread of the present dataset}
head(present)
```

From the head, we can see that the columns in this dataset are:
  - year
  - boys
  - girls
  
similar to the Arbuthnot data set.

To see the size of the dataset, we can use the glimpse function as before:
...
To get the years included in this dataset, I'll use the summarize function that was just introduced replacing min(boys) and max(boys) with years.
```{summarizing the present dataset}
present %>%
...
**Insert your answer here**
In counts, this dataset has the same number of columns but fewer years being observed, i.e. fewer rows.

Regarding the magnitude, let's use a plot to answer this question. We will plot both of these data sets. Since our question is simply to answer to the magnitude, let's graph both of these datasets using the index as the x axis. This would make it so that we can pretty easily see how large the gap between the data sets are.

To do this, we will first need the total number of births added as a column on the present data set:

```{calculating total for the present dataset}
present <- present %>%
  mutate(total = boys + girls)
...
```{plotting both datasets on one chart with the index as the x axis}
...
Let's be honest. If you had asked me prior to looking at data if the magnitude of births from London in the 17th century to the **total number of births in the USA in the 19th century** I would have instinctively known it wasn't (although this was good practice). The graph generated very much so supports this. The red line (representing Arbuthnot's data) barely registers on when sharing a scale with the total births in the USA in the 19th century.

Now, looking for a specific answer to the question of how large the magnitude is:

```{calculating how large the magnitude of difference is}
...
```{graphing the the proportion of boys for the present data set}
...
This seems to also align with Arbuthnot's observation. For every year in the present dataset, we can see that boys consistently have more births than girls.

To see this with Arbuthnot's data, let's plot a graph where we can see both of these:

```{plotting both datasets proportion of boy births on the same graph.}
present_vs_arbuthnot = ggplot() + 
...
I appreciate the suggestion, but I prefer to do it in the following way:
  ...
  2) Filter the dataset to the entry with the max total
  
With those 2 steps, we can reliabily get the max in the dataset and present it

```{getting the year in the present dataset with the max births}
present <- present %>%
  mutate(total = boys + girls)
