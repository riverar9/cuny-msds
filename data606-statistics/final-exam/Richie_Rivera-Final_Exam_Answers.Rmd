...
Looking visually, the data looks (like a dinosaur) as though there is no linear correlation here. We can also see that with a correlation coefficient of `r data1.correlation` which is very close to 0. A score close to 0 would mean that there isn't linear relationship between the data.

Additionally, because the slope of this graph is so close to 0 (`r data1.slope`) the residuals resemble the scatterplot.
...
Looking visually at the data, we can see that there seems to be a strong linear correlation between multiple bands of categories of data. Since we only have the x and y, it's likely not a good candidate for linear regression since any given x could have a wide array of potential y values. This can also be seen with the correlation coefficient of `r data2.correlation` which is also very close to 0 suggesting there is there isn't linear relationship between the data.
...
Here we are in a similar situation as model1 and model2. Both the scatterplot and residuals plot provide evidence that a linear model isn't appropriate. Additionally, a close to 0 correlation coefficient of `r data3.correlation` also provides evidence that a linear model here isn't appropriate.
